...
# IOT_FLEET_TOKEN is required as well — the setup registers a throw-away
# device, and first registrations need a valid X-Provisioning-Token.
#
# Usage:
...
EOF
    ...
    local resp
    # Seit dem Fleet-Token-Modell (post-iot-register.php, handleNewRegistration)
    # verlangt die Erst-Registrierung einen gueltigen X-Provisioning-Token —
    # ohne ihn antwortet der Server mit 401 und das Setup scheitert.
    resp=$(curl -s -X POST "${BASE_URL}/iot/register" \
        -H "Content-Type: application/json" \
        -H "X-Provisioning-Token: ${IOT_FLEET_TOKEN}" -d "$payload")
